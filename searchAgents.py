  ...
  def getSuccessors(self, state):
    ...
    for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
      ...
      if not self.walls[nextx][nexty]:
        ...
        #Update visited corners if we reach a new corner
        next_visited = list(visited_corners)
        for i, corner in enumerate(self.corners):
          if nextPosition == corner:
            next_visited[i] = True
            break
        nextState = (nextPosition, tuple(next_visited))
...
def cornersHeuristic(state, problem):
  ...
  # Find  the maximmum Manhattan distance from current position to any unvisited corner
  max_distance = 0
...
def foodHeuristic(state, problem):
  ...
  distances = [util.manhattanDistance(position, food_pos) for food_pos in food_list]
